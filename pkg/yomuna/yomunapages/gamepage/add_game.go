...
func (ui *AddGameUI) layoutPathField(gtx layout.Context) layout.Dimensions {
	return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return theme.ThemedLabel(gtx, ui.th, ui.theme, theme.TextRoleLabel, theme.ThemeColorTextPrimary, "Game path")
		}),
		...
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return layout.Flex{Axis: layout.Horizontal, Alignment: layout.End}.Layout(gtx,
				layout.Flexed(1, func(gtx layout.Context) layout.Dimensions {
					...
					oldLabel := ui.pathInput.Label
					ui.pathInput.Label = ""
					dims := ui.pathInput.Layout(gtx)
					ui.pathInput.Label = oldLabel
					return dims
				}),
				layout.Rigid(layout.Spacer{Width: unit.Dp(8)}.Layout),
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					if ui.browseButton.Clicked(gtx) {
						ui.openFilePicker()
						gtx.Execute(op.InvalidateCmd{})
					}
					return ui.browseButton.Layout(gtx)
				}),
			)
		}),
	)
...
func (ui *AddGameUI) layoutActions(gtx layout.Context) layout.Dimensions {
	...
	if ui.clearButton != nil {
		ui.clearButton.Disabled = ui.installing || strings.TrimSpace(ui.pathInput.Text()) == ""
	}
	...
	return layout.Flex{Axis: layout.Horizontal, Alignment: layout.Middle}.Layout(gtx,
		...
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			if ui.clearButton.Clicked(gtx) && ui.pathInput != nil {
...
func (ui *AddGameUI) openFilePicker() {
	if ui == nil || ui.pathInput == nil {
		return
	}
	startDir := filePickerStartDir(ui.pathInput.Text())
